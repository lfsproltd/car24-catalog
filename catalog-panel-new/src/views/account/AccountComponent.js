      ...
      oktaAuth.getUser().then((info) => {
        ...
        if (localStorage.getItem("okta-token-storage")) {
          ...
          if (
            ...
            userClaims.accessToken.claims
          ) {
            if (
              userClaims.accessToken.claims.country &&
              userClaims.accessToken.claims.country.length &&
              userClaims.accessToken.claims.vehicleType &&
              userClaims.accessToken.claims.vehicleType.length
            ) {
              let country = "";
              if (
                userClaims.accessToken.claims.country &&
              ...
              if (
                ...
                if (roles && roles.includes("CATALOG_QA")) {
                  ...
                  (roles && roles.includes("CATALOG_WSM")) ||
                  roles.includes("CATALOG_TECH")
                ) {
                  historyLink.push({
                    pathname: "/estimate",
                    state: {
