...
const EstimateDetailsComponent = (props) => {
  ...
  let estimateFields = {};
  let worktobedone = {};
  const [estimatesFieldsInitial, setEstimatesFieldsInitial] =
    useState(estimateFields);
  ...
  useEffect(() => {
    if (
      ...
      props.listingDetailsUpdated[0] &&
      props.listingDetailsUpdated[0].data &&
    ...
    if (
      ...
      Object.keys(props.listingDetailsUpdated[0].data.qualityChecks).map(
        (item) => {
          //check whether acceptable imperfection is rejected
          if (
            props.listingDetailsUpdated[0].data.qualityChecks[item]
              ?.invalidated === false &&
            props.listingDetailsUpdated[0].data.checkpoints[item]?.ok ===
              true &&
            props.listingDetailsUpdated[0].data.checkpoints[item]?.choices
              .length &&
            props.listingDetailsUpdated[0].data.qualityChecks[item]?.status ===
              "REJECTED"
          ) {
          ...
          if (
            props.listingDetailsUpdated[0].data.checkpoints[item]?.ok ===
              false &&
            props.listingDetailsUpdated[0].data?.qualityChecks[item]?.reason ===
              "NO_WORK_TO_BE_DONE" &&
            props.listingDetailsUpdated[0].data?.qualityChecks[item]?.status ===
              "APPROVED"
          ) {
          ...
          if (
            props.listingDetailsUpdated[0].data?.estimates[item] &&
            props.listingDetailsUpdated[0].data?.estimates[item]
              ?.invalidated === false
          ) {
  ...
  useEffect(() => {
    ...
    if (props && props.listingDetails && props.listingDetails[0]) {
      Object.keys(props.listingDetails[0].data.qualityChecks).map(
        (item, index) => {
          if (
            props.listingDetails[0].data.qualityChecks[item] &&
            props.listingDetails[0].data.qualityChecks[item]?.invalidated ===
              false &&
            props.listingDetails[0].data.checkpoints[item]?.ok === false
          ) {
            allValidQualityChecks += 1;
            if (
              props.listingDetails[0].data.estimates[item] &&
              props.listingDetails[0].data.estimates[item]?.invalidated ===
                false
            ) {
              totalEstimtates += 1;
              estimateFields[item] = {
                labourCost:
                  props.listingDetails[0].data.estimates[item].labourCost >= 0
                    ? props.listingDetails[0].data.estimates[item].labourCost
                    : null,
                parts: props.listingDetails[0].data.estimates[item].parts
                  ? props.listingDetails[0].data.estimates[item].parts
                  : [{ name: "", cost: null }],
              };
              if (
                props.listingDetails[0].data.estimates[item].parts &&
                props.listingDetails[0].data.estimates[item].parts.length === 0
              ) {
                props.listingDetails[0].data.estimates[item].parts = [
                  { name: "", cost: null },
                ];
              ...
              refurbishment[item] = {};
              if (
                props.listingDetails[0].data.checkpoints[item]
                  ?.refurbishmentChoices
              ) {
                refurbishment[item] =
                  props.listingDetails[0].data.checkpoints[
                    item
                  ]?.refurbishmentChoices;
              }
            } else {
    ...
    if (
      ...
      props.listingDetails[0].data.estimates
    ) {
      ...
      if (
        props.listingDetails &&
        props.listingDetails[0] &&
        props.listingDetails[0].inspectionStatus &&
  ...
  useEffect(() => {
    ...
    let totalEstimate = 0;
    props &&
      props.listingDetails.map((data) => {
        Object.keys(data.data.qualityChecks).map((item) => {
          if (
            data.data.qualityChecks[item].invalidated === false &&
            data.data.checkpoints[item].ok === false &&
            data.data.qualityChecks[item].status !== "APPROVED"
          ) {
            totalEstimate += 1;
          }
        });
  ...
  const editClickHandler = (index, item) => {
    ...
    if (
      listingDetail &&
      listingDetail.data &&
      listingDetail.data.checkpoints[item] &&
      listingDetail.data.checkpoints[item].refurbishmentChoices
    ) {
      availableData[item] = {
        refurbishmentChoices: [],
      };
      listingDetail.data.checkpoints[item].refurbishmentChoices.map(
        (choice1) => {
          listingDetail.data.checkpoints[item].refurbishmentChoices.map(
            (choice1) => {
          ...
          availableData[item].refurbishmentChoices.push({
            other: choice1.other,
            refurbishment: choice1.refurbishment,
            checked: true,
          });
        }
      );
  ...
  const addtionalPartNameHandler = (e, item, partIntex) => {
    if (
      ...
      estimatesFieldsInitial[item].parts = [
        {
          name: estimatesFieldsInitial[item]?.parts[partIntex]?.name
            ? estimatesFieldsInitial[item]?.parts[partIntex]?.name
            : "",
          cost: estimatesFieldsInitial[item]?.parts[partIntex]?.cost
            ? estimatesFieldsInitial[item]?.parts[partIntex]?.cost
            : null,
        },
      ];
    }
  ...
  const additionalPartCostHandler = (e, item, partIntex) => {
    ...
    estimatesFieldsInitial[item].parts[partIntex].cost = e.target.value
      ? e.target.value
      : "";
    setEstimatesFieldsInitial({ ...estimatesFieldsInitial });
  ...
  const onSaveEditsHandler = (index, data) => {
    ...
    if (
      sendSingleEstimate[data].parts &&
      sendSingleEstimate[data].parts.length
    ) {
      ...
      if (
        estimatesFinalData.data.estimates[data] &&
        !!estimatesFinalData.data.estimates[data].labourCost
      )
        estimatesFinalData.data.estimates[data].labourCost =
          +estimatesFinalData.data.estimates[data].labourCost;
    }
    ...
    if (editedRefurbs && editedRefurbs.length) {
      ...
      estimatesFinalData.data.estimates[data]["refurbishmentChoices"] =
        uniqRefurbs;
    }
    // submit data to api
    console.log(estimatesFinalData);
    dispatch(
      props.approveQualityChecks(
        estimatesFinalData,
        listingDetail.appointmentId,
        "Estimate filled successfully"
      )
    );
    setIsEdit("");
    setOtherWorkToBeDoneFieldValue("");
            ...
            {listingDetail && listingDetail.make && (
              <div className="col-lg-3">
                <div className="form-group">
                  ...
                  <span>
                    {listingDetail.variant ? listingDetail.variant + " | " : ""}
                  </span>
                  <span>
                    {listingDetail.fuelType ? listingDetail.fuelType : ""}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="row contentWrapper">
            ...
            <div className="col-lg-3">
              <div className="form-group bolder-span">
                ...
                <span>
                  {listingDetail?.formatedUpdatedAt
                    ? " " + listingDetail.formatedUpdatedAt
                    : " N/A"}
                </span>
              </div>
              ...
              <div className="col-lg-12 text-center">
                <div className="form-group bolder-span">
                  <span style={{ color: "#F34500" }}>
                    Acceptable imperfections have been rejected for this
                    appointment id. Please re inspect them through mobile app.
                  </span>
                </div>
              </div>
            ) : null}
            ...
            <div className="col-lg-12">
              <div className="container-box estimates-container">
                {listingDetail && listingDetail.data && (
                  <>
                    {Object.keys(listingDetail.data.qualityChecks).map(
                      (item, index) => {
                        return (
                          <>
                            {listingDetail.data.qualityChecks[item]
                              ?.invalidated === false &&
                              listingDetail.data.checkpoints[item]?.ok ===
                                false && (
                                <div className="container-box-card mt-4">
                                  <label className="label-top">{item}</label>
                                  <label className="label-top status-right-aligned">
                                    {listingDetail.data.qualityChecks[item]
                                      ?.reason === "NO_WORK_TO_BE_DONE" &&
                                    listingDetail.data.qualityChecks[item]
                                      .status === "APPROVED"
                                      ? "No work to be done"
                                      : listingDetail.data.qualityChecks[
                                          item
                                        ].status.toLowerCase()}
                                  </label>
                                      ...
                                      <div className="col-lg-12">
                                        <div className="form-group photos-row">
                                          <div className="estimate-photo-container-horiz">
                                            {listingDetail.data.checkpoints[
                                              item
                                      ...
                                      <div className="col-lg-6">
                                        ...
                                        <div className="col-lg-12 light-label dark-span">
                                          ...
                                          <input
                                            ...
                                            onChange={(e) =>
                                              estimatedLaborCostHandler(e, item)
                                            }
                                            disabled={
                                              isEdit !== index ||
                                              listingDetail.data.qualityChecks[
                                      ...
                                      <div className="col-lg-6">
                                                    ...
                                                    <div className="col-lg-2">
                                                      ...
                                                      partIndex !== 0 ? (
                                                        <button
                                                          ...
                                                          className="remove-additional-part"
                                                          onClick={(e) =>
                                                            removeParts(
                                                              item,
                                                              partIndex
                                        ...
                                        {isEdit === index && (
                                          <button
                                            ...
                                            disabled={
                                              !!!estimatesFieldsInitial[item]
                                                .parts[prevIndex]?.cost ||
                                              !!!estimatesFieldsInitial[item]
                                                .parts[prevIndex]?.name
                                            }
                                          >
                                            Add More
                                          </button>
