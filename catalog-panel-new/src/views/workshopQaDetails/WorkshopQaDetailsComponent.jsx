...
//  const [masterData, setMasterData] = useState({});
  ...
  useEffect(() =>{
    console.log("starting data calculations");
    ...
    if(props && props.listingDetails && props.listingDetails[0] && props.listingDetails[0].appointmentId){
     ...
     // GetInspectionSummaryAction(searchParams) // TODO: Check If need to uncommend
     // setInspectionData(props.lastInspectionData[0]);
      }
    if(props && props.listingDetails[0] && props.listingDetails[0].data && props.listingDetails[0].data.checkpoints){
      Object.keys(props.listingDetails[0].data.checkpoints).map((item)=>{
          masterData.qaTopImages.map((itemTop)=>{
            if(itemTop === item){
  ...
  },[props.listingDetails, masterData]);
  ...
  */
  ...
  const onFinalSubmitNoWorkToBeDone = (item) => {
    let data = {
      ...
      "data":{
          ...
          ApproveQualityChecksAction(data,appointmentDetails.appointmentId,"Approved Successfully");
          let params = {
          ...
          setTimeout(()=>{
            ...
            //dispatch(getWorkshopListingDetails(params)); Why Two times
            GetWorkshopQaListingDetailsAction(params)
          },1000);
  
          setTimeout(()=>{
            //GetInspectionSummaryAction({appointmentId:appointmentDetails.appointmentId}) // Need to check
  ...
  const onSubmitReject = (item,index,comment,optional)=>{
    ...
    if(getConfirmation && confirmationForOnSubmitReject.isConfirm){
      if(appointmentDetails && appointmentDetails.inspectionStatus === "ESTIMATED"){
        ...
        ApproveQualityChecksAction(qcActionData,appointmentId,"Rejected successfully")
  ...
  const onSubmitApprove = (item,index,optional) =>{
    ...
    if(appointmentDetails && appointmentDetails.inspectionStatus === "ESTIMATED"){
      let appointmentId = window.location.pathname.split("/")[2];
      qcActionData.locationType = appointmentDetails.locationType;
      ApproveQualityChecksAction(qcActionData,appointmentId,"Approved successfully");
  ...
  const submitAllQcAction = () => {
    ...
    ApproveQualityChecksAction(payload,appointmentId,"Submitted successfully","submit all qc workshopQA");
  ...
  const updateCheckpointDataHandler = (dataKey) => {
      ...
      let noImperfectionChoices = [];
        ...
        setTimeout(()=>{
        //Action({appointmentId:appointmentDetails.appointmentId}); // Need to check
          //dispatch(getInspectionSummary({appointmentId:appointmentDetails.appointmentId}));
          //dispatch(getInspectionSummary({appointmentId:appointmentDetails.appointmentId}));
        },1100);
        setGetConfirmation(false);
        ...
        <div className="row contentWrapper border-top">
          {/* <div className="col-lg-3">
            ...
            <div className="col-lg-4">
              <div className="form-group bolder-span cost-center">
              <label>{labels['PENDING_ESTIMATE_COST']}: </label>
        ...
        <div className="row titleWrapper">
          <div className="col-lg-12">
            <div className="photos-row align-items-center d-flex justify-content-between">
              <label>{labels['PHOTOS']}</label> <span>{labels['TOTAL']} <strong>{allImages.length}</strong> {labels['PHOTOS']} </span>
            </div>
                      ...
                      <div className="row">
                        <div className="col-lg-6">
                          ...
                          appointmentDetails.data.checkpoints[item].images.length > 0 && (
                            <button className="toggle-view-button" onClick={()=>{toggleImageHandler(index)}}>{labels['IMAGE_VIEW']}</button>
                          )}
                          {appointmentDetails.data.checkpoints[item] && 
                          appointmentDetails.data.checkpoints[item].videos &&
                          appointmentDetails.data.checkpoints[item].videos.length > 0 && (
                            <button className="toggle-view-button" onClick={()=>{toggleVideoHandler(index)}}>{labels['VIDEO_VIEW']}</button>
                          )}
                        </div>
                        <div className="col-lg-6">
                          <div className="row">
                              ...
                              appointmentDetails.data.checkpoints[item].choices.length > 0 &&(
                                <div className="col-lg-12  light-label dark-span textTitle">
                                      <h6>{labels['UNACCEPTABLE_IMPERFECTION_SHOW_TO_CUSTOMER']}</h6>
                              ...
                              appointmentDetails.data.checkpoints[item].choices.length  > 0 &&(
                                ...
                                <div className="col-lg-12 light-label dark-span textTitle">
                                    ...
                                    <div className="col-lg-12 light-label dark-span textTitle">
                                    <h6>{labels['ACCEPTABLE_IMPERFECTION_NOT_SHOWN_TO_CUSTOMER']}</h6>
                                    {appointmentDetails.data.checkpoints[item].choices.map((choice)=>{
                                      return(choice.acceptable ? choice.choice + ' | ' : '')
                                      })}
                                  </div>
                            ...
                            {isRejected === index ? (
                              <div className="row"> 
                                <div className="col-lg-12 button-container buttonContainer">
                                  ...
                                  setIsEditCheckpoint("");setDataKey("");rejectHandler(item)}}>{labels['REJECT']}</button>
                                  <button 
                                  style={{backgroundColor: isConfirmationDialog ? 'grey' : '#F37500'}}
                                  disabled={isConfirmationDialog || !iscorrectAssigned}
                                  onClick={(e)=>{onSubmitApprove(item,index,false);}}>{labels['APPROVE']}</button>
                                  {appointmentDetails.data.checkpoints[item] && appointmentDetails.data.checkpoints[item].ok === false && 
                                  appointmentDetails.data.checkpoints[item].choices.length  > 0 && (
                            ...
                            <div className="row">
                              <div className="col-lg-6">
                                ...
                                lastInspectionData.data.checkpoints[inspectionItem].images.length > 0 && (
                                  <button className="toggle-view-button" onClick={()=>{toggleImageHandler2(index2,"image")}}>{labels['IMAGE_VIEW']}</button>
                                )}
                                {lastInspectionData.data.checkpoints[inspectionItem] && 
                                ...
                                lastInspectionData.data.checkpoints[inspectionItem].videos.length > 0 && (
                                  <button className="toggle-view-button" onClick={()=>{toggleVideoHandler2(index2,"video")}}>{labels['VIDEO_VIEW']}</button>
                                )}
                              </div>
                              <div className="col-lg-6">
                                <div className="row">
                                  ...
                                  (
                                    ...
                                    <div className="col-lg-12 light-label dark-span">
                                        ...
                                        <div className="row">
                                          <div className="col-lg-12 light-label dark-span textTitle">
                                            <br/>
                                            <span className="small-text">{labels['OTHERS']} : </span><br/>
                                            <input type="text" className="form-control"
                                            value={otherImperfectionEntered} 
                                            onChange={e=> otherImperfection(e.target.value)}/>
                                          </div>
                    ...
                    {isRejected === index ? (
                      <div className="col-lg-12 button-container buttonContainer">
                        <div className="row">
                          ...
                          onClick={()=>{setRejectAction(index);setIsConfirmationDialog(false);setIsEditCheckpoint("");setDataKey("");rejectHandler(item)}}>{labels['REJECT']}</button>
                          </div>
                        ) : (
                          <div className="col-lg-12 button-container buttonContainer">
                                    {appointmentDetails.data.qualityChecks[item] &&
...
export default WorkshopQaDetailsComponent;